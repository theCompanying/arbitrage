...
export class AliExpressApiService {
  ...
  private parseProductItem(item: Record<string, unknown>): AliExpressProduct {
    return {
      ...
      images: (item.image_urls as string[]) || [],
      price: {
        min: parseFloat(item.min_price as string) || 0,
        max: parseFloat(item.max_price as string) || 0,
        currency: (item.currency as string) || 'USD',
        originalPrice: parseFloat(item.original_price as string) || 0,
        discountPercent: parseFloat(item.discount_percent as string) || 0,
      },
      shipping: {
        cost: parseFloat(item.shipping_cost as string) || 0,
        freeShipping: item.free_shipping === true || item.free_shipping === 'true',
        estimatedDeliveryDays: item.delivery_time ? {
          min: (item.delivery_time_min as number) || 0,
          max: (item.delivery_time_max as number) || 0,
        } : undefined,
        shippingFrom: item.shipping_from as string,
      },
      supplier: {
        sellerId: (item.seller_id as number) || 0,
        sellerName: (item.seller_name as string) || '',
        storeUrl: (item.store_url as string) || '',
        positiveRating: parseFloat(item.positive_rating as string) || 0,
        totalTransactions: (item.total_transactions as number) || 0,
        yearsInBusiness: (item.years_in_business as number) || 0,
      },
      orders: (item.orders as number) || 0,
      rating: parseFloat(item.rating as string) || 0,
      reviews: (item.reviews as number) || 0,
      moq: (item.moq as number) || 1,
      availableStock: item.available_stock as number,
      variants: (item.variants as unknown[] as Record<string, unknown>[])?.map((v) => ({
